...
parser.add_argument('--uc', help='Path to unit cell')
parser.add_argument('--uc2', default='', help='Path to second unit cell')
...
parser.add_argument('--interpolate', type=float, default=0.0, help='Percentage of second structure to mix')
...
def loopable(structure_id, UEDavg = None):
    ...
    if args.add_degradation:
        vac_fraction = np.random.randint(95,99)
    else:
        vac_fraction = 0
    ...
    structure = gen_structure(args.uc, particle_size, vac_fraction, thermal_displacement, shear_strain, rotation_angle)

    if args.interpolate > 0.01:
        # particle_size = np.rint(np.random.normal(1,0.2,2) * np.array([51,30])).astype(int)
        # vac_fraction = np.random.randint(0,20)
        # thermal_displacement = np.random.uniform(low = 0.0, high = 0.3)
        # shear_strain = np.random.normal(1,0.09,2)
        # rotation_angle = np.random.uniform(low = 0.0-(np.pi/3.0), high = np.pi/3.0)
        structure_2 = gen_structure(args.uc2, particle_size, vac_fraction, thermal_displacement, shear_strain, rotation_angle)

    intensity_level = np.random.gamma(2,1)
    intensity_level = np.maximum(intensity_level, 0.1)  # Set intensity level to at least 0.1
    if args.interpolate > 0.01:
        UED = make_ued_pattern(structure.snaplist[0], intensity_level, structure_2.snaplist[0], args.interpolate)
    else:
...
if __name__ == "__main__":
    ...
    # # If you want to subtract from a reference structure
    # if args.ucref != '':
    #     UEDavg = []
    ...
    #         UED = make_ued_pattern(structure.snaplist[0], intensity_level)
    #         UEDavg.append(UED)
    #     UEDavg = np.mean(UEDavg, axis=0)
    #     UEDavgmax = 0.25 * np.max(UEDavg)

    # for structure_id in range(args.num):
    ...
    #     # UED[UED > 255] = 255
    #     # UED[UED < 0] = 0
    #     # UED = UED.astype(np.uint8)
    #     # image = Image.fromarray(UED, mode='L')
    #     # image.save(filepath, dpi=sizes)
